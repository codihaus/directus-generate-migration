...
module.exports = class FieldClass {
	...
	constructor(knex) {
		...
		this.fieldDefault = {
			...
			code: (type,options) => this.generateNormal(type ?? "json" , {
				meta: {
			...
			files: (temp_collection,options) => this.generateM2m("directus_files" ,temp_collection, {
				meta: {
					interface: "files" ,
					special: ["files"] ,
					...(options?.meta || {})
			...
			translations: (collection_language,temp_collection, fields, options) => this.generateM2m(collection_language ,temp_collection, {
				meta: {
	...
	generateM2m(related_collection, temp_collection , options , fields_extend) {
		return {
			//type: "alias",
			type: "$M2M$" ,
			meta: {
